...
```
 ...
 │  (builds SQL)       │   │  (builds script file)   │
 │                     │   │                          │
 │  buildSQL(config)   │   │  buildScript(config)     │
 │  describeRules(cfg) │   │  → run.py or            │
...
The `diabetes_nephropathy/` folder is a placeholder for a sample package.
It is currently empty. Use the wizard’s diabetes example preset and click
**Generate Package** to produce sample output.
